...
`author_ref`、`critic_ref` 或 `integrator_ref`。实现 lane 只使用这些工作区/集成字段：
...
`repository_identity`、`candidate_anchor`、
...
`coder_ref` 是该 lane 的 writer agent，三者不能互相替代。`run_id` 仍只表示执行来源，不是唯一键。
...
`workspace-prepared` 前，都运行 `git rev-parse --show-toplevel`，并要求解析结果等于绝对
...
- `workspace-prepared`：已分配工作区、仓库根核验和精确的
...
- `integrating`：Integrator 正在把候选跨隔离工作区、并发 writer 或共享基线边界应用。
...
`ownership_epoch`、`coder_ref` 和路径。同一卡或同一 canonical 路径最多有一个活动 writer。跨任务
`claim` 不携带 Coder 身份，随后用 `bind-coder` 显式绑定一个；每次 verify、anchor 和正常 release
都必须携带并精确匹配该 `coder_ref`。绑定前只能用 `cancel-unbound` 取消 claim。跨任务扫描只用于
诊断冲突，不是锁。旧 owner、旧 epoch、缺失/错误 Coder 或重建仓库路径的回传必须在审查、集成前
拒绝。Reviewer、Verifier 只能按已登记锚只读共存。
...
`critic_ref`。代码 finding、`integration-conflict` 和
...
registry 或派 Reviewer。scheduler 核验已绑定 lane/仓库/路径、候选 commit 和 `write_set`，执行原子
anchor，再发送绑定候选与 epoch 的 `review-authorized`。只有此后 worker 才能派 Reviewer；修订会
使旧授权失效，并重走该门禁。
...
- **Verifier**：按当前阶段独立运行测试、真实路径、负向场景和门禁。当前派发为 run-task 卡时，
  ...
  两次都核验当前派发路径。
- **Integrator**：文档候选跨集成边界时，或里程碑关账阶段明确要求时，执行已接受的机械传导。
  当前派发为 run-task 卡时，作为共享基线单一写者并执行上述两阶段集成与回滚协议。
...
- **Codex**：每次派发都携带这些要求，记录 target/thread，并把修订或复核 follow-up 发回原
  ...
  scheduler 执行 `claim → bind-coder → verify`，再发激活消息，让 worker 恢复同一 Coder 写入。

  原 scheduler 是全局 ready set、lane registry、integration queue、共享基线与 Integrator 的唯一
  ...
  分组；支持并行等待就并行，不支持则先对各组做 `timeoutMs: 0` 快照，再轮转有界等待。任一完成后
  立即补位。缺能力或未获本轮授权时，退化为当前任务内滚动调度。不得把当前/默认 subagent 数或
  wait target 数写成方法常量。
- **Claude Code**：普通 subagent 默认共享主会话 cwd，也不共享调度 DAG 或自动 ready set。主
  ...
  Author、Critic、Coder、Reviewer、Verifier、Integrator 工作委派给需要恢复身份的 agent 时，
  ...
  GMGN 不依赖自动 merge。候选跨集成边界时由已登记 Integrator 显式集成；实现候选始终如此。
  原生 worktree 默认基线可能是
