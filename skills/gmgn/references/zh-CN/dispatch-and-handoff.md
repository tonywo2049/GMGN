...
`coder_ref` 是该 lane 的实际 writer：通常是受委派 Coder；满足“当前无法与主编排工作并行”规则时
也可以是主 session。三者不能互相替代。`run_id` 仍只表示执行来源，不是唯一键。写入前绑定实际
Coder，禁止静默接管已绑定给其他 Coder 的 lane。
...
- **Coder**：只实现一张已批准卡，只在记录工作区和 `write_set` 内工作；在 detached `HEAD` 或
  ...
  禁止远端写入或夹带无关路径。只有当前没有实现 lane 能与有用的主编排工作并行时，主 session
  才可在一条 lane 上承担该身份；隔离、锚定、独立 Reviewer 与独立 Verifier 均不豁免。
...
- **Codex**：每次派发都携带这些要求，记录 target/thread，并把修订或复核 follow-up 发回原
  ...
  最长安全等待阻塞一个组，真实超时后才轮换。超时只是存活检查点，不得自动列出/读取任务后再次
  等待。任一实质更新后立即补位；worker 主动推送 blocker、候选、审查、验证和完成事件，不发周期
  heartbeat。缺能力或未获本轮授权时，退化为当前任务内滚动调度。不得把当前/默认 subagent 数、
