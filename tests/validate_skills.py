...
def validate_telemetry_contract(
    ...
    require_section(
        ...
        (
            ...
            "success/exit status, classifications, wait outcome, fork policy",
            "Codex `/hooks`",
            "unstable fallback",
            "Every metric reports its source and coverage",
            "Missing actual token data is `unknown`, not zero",
            "wait outcomes", "maximum consecutive timeouts", "wait-storm count",
            "merged per `tool_use_id`", "session JSONL fills only uncovered calls",
            "Legacy unstructured rejection output", "message wording",
            "session_sequence_delta", "matched/eligible coverage",
    ...
    require_section(
        ...
        (
            ...
            "成功/退出状态、分类、等待结果、fork policy",
            "Codex `/hooks`",
            "unstable fallback",
            "每个指标都报告来源与 coverage",
            "actual token 缺失时显示 `unknown`",
            "等待结果", "最大连续超时", "等待风暴数", "session_sequence_delta",
            "按 `tool_use_id` 逐次合并", "session JSONL 只补未覆盖调用",
            "旧版非结构化", "消息文案",
            "matched/eligible coverage",
    ...
    telemetry_sections = (
        (
            ...
            (
                ...
                "normalized outcome", "consecutive-timeout", "wait-storm",
                "per `tool_use_id`", "only for uncovered calls", "message wording",
                "session_sequence_delta", "matched/eligible coverage",
        ...
        (
            ...
            (
                ...
                "失败不阻塞交付",
                "只有用户明确要求复盘",
                "归一化结果", "连续超时", "等待风暴", "session_sequence_delta",
                "按 `tool_use_id` 逐次合并", "只补未覆盖调用", "消息文案",
                "matched/eligible coverage",
        ...
        (
            ...
            (
                ...
                "only when the user explicitly requests a retrospective",
                "normalized result", "wait-storm", "session_sequence_delta",
                "matched/eligible coverage",
                "does not change DocStar or its JSON output",
                "fresh full rebuild on every invocation",
                "no cache",
        ...
        (
            ...
            (
                ...
                "Telemetry failure never blocks delivery",
                "only when the user explicitly requests a retrospective",
                "normalized outcome", "wait-storm", "session_sequence_delta",
                "matched/eligible coverage",
                "does not change DocStar or its JSON output",
...
def validate_agent_lifecycle(
    ...
    adapter_tokens = {
        dispatch_paths[0]: (
            ...
            "longest platform-safe wait", "liveness checkpoint", "list_agents",
            "next external reactivation", "current activation must not call another wait",
            "task-derived liveness threshold", "periodic heartbeats",
        ...
        dispatch_paths[1]: (
            ...
            "timeoutMs: 0",
            "最长安全等待", "存活检查点", "list_agents", "任务本身推导的存活阈值",
            "下一次外部重新激活", "本轮不得", "立即 yield",
            "周期 heartbeat",
    ...
    require_tokens(
        ...
        (
            ...
            "no implementation lane can currently run in parallel with useful orchestrator work",
            "one ready lane and no useful orchestration work can run beside it",
            "primary session as `coder_ref`", "independent Reviewer", "independent Verifier",
            "scripts/lane_registry.py", "update-ref",
            "node-complete", "cancel-unbound", "candidate-awaiting-anchor",
            "review-authorized", "clientThreadId", "threadId", "hostId", "timeoutMs: 0",
            "one `wait_agent` covering any live agent", "liveness checkpoint",
            "do not automatically chain `list_agents`", "periodic heartbeats",
        ),
        "run-task",
    )
    ...
    require_section_tokens(
        ...
        (
            ...
            "longest platform-safe wait", "liveness checkpoint", "periodic heartbeats",
            "do not infer or hard-code a subagent, wait-target, or polling interval limit",
            "no implementation lane that can run in parallel with useful orchestrator work",
            "bind the primary session as `coder_ref`", "independent",
        ),
        "run-task 滚动补槽",
    )
    require_section_tokens(
    ...
    for path, tokens in (
        (
            ...
            (
                ...
                "primary session only when no implementation lane can currently run in parallel",
                "event-driven `wait_agent`", "liveness checkpoint", "polling interval",
        ...
        (
            ...
            (
                ...
                "review-authorized",
                "当前没有实现 lane 能与有用的主编排工作并行", "事件驱动 `wait_agent`",
                "存活检查点", "轮询间隔",
