...
anchors, allowed files, exclusions, checks, and return format. Its caller resolves semantic
fields under the active workflow; the brief supplies runtime and workspace facts. Use only
for a `brainstorm`, `write-*`, `roadmap`, or `close-milestone` document candidate; normal
...
caller. Do not create other agents.
...
Owner feedback or the caller's finding ruling. Apply an accepted in-scope fix in the same
dispatch while the objective and write boundary remain unchanged; otherwise require
