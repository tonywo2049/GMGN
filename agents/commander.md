...
workspace, and lock rules. Do not redefine upstream meaning in execution. When an upstream
change is required, invoke its owning Skill inside this dispatch, create its required roles,
...
candidate content changes, do not advance the baseline; return the exact repair and invalidated
gates so the primary orchestrator can resume the applicable Runner, then continue this same
Commander
matter.
