...
| 风险触发的最终验证 | 最终制品、安装、启动、E2E 或外部环境命令 | 项目命令；可用 `/verify` |
...
`Task.md` 和追踪矩阵。受委派 Coder 回传候选的原始 base 和当前 tip；应用完整 base-to-tip diff，
不得只应用最后一个修订提交。主 session 单独承担 Coder 时可以冻结并 hash 精确 diff。隔离 lane
候选在唯一审查轮前应用到临时组合；单 writer 候选已基于未变共享基线时，本身就是组合。机械组合
只改变 commit SHA 时不使证据失效；应比较相关内容。Task 状态、描述性 Log 和无关任务行本身不使
证据失效。execution 指针只有在仍解析到同一规范 Card，或该 Card 的权威锚、完成条件和 TDD 契约
不变时才等价。Reviewer 在唯一审查轮内运行既定的确定性本地检查；已采纳
