...
原生审查不替代真实运行。每个受委派角色在创建前取得完整 brief，只回传一次后结束；后续写作、
编码或验证使用全新 agent，不继承父会话或旧 agent 历史。每个语义变更批次或任务执行最多只有一轮
Critic/Reviewer；已采纳 finding 由主编排者核对修复，不再做第二轮审查。
...
已基于未变共享基线时，本身就是组合。已采纳 finding 修复后，需要可执行证据时只派一个全新
Verifier 验证最终候选；修复不再交给 Reviewer，干净机械集成也不重复同一测试。
